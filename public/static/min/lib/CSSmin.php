...
class CSSmin
{
    ...
    private function do_raise_php_limits()
    {
        ...
        foreach ($php_limits as $name => $suggested) {
            ...
            if ($current > -1 && (-1 == $suggested || $current < $suggested)) {
    ...
    private function minify($css, $linebreak_pos)
    {
        ...
        if (false !== $linebreak_pos && (int) $linebreak_pos >= 0) {
    ...
    private function extract_data_urls($css)
    {
        ...
        while (preg_match($pattern, $css, $m, 0, $offset)) {
            ...
            if (0 === strlen($terminator)) {
                $terminator = ')';
            }

            while (false === $found_terminator && $end_index + 1 <= $max_index) {
    ...
    private function str_slice($str, $start = 0, $end = false)
    {
        ...
        $slice = (false === $end) ? substr($str, $start) : substr($str, $start, $end - $start);
