@extends('layouts.app')
@section('title', __('adnetwork.fb_users'))
@section('content')
    <div class="content-inner">
        <div class="breadcrumb">
            <ul>
                <li><a href="{{route('home', app()->getLocale())}}">Smartbee</a></li>
                <li><span>{{__('adnetwork.fb_users')}}</span></li>
            </ul>
        </div>
                        ...
                        <div class="cols w-mob">
                            ...
                            <div class="col-item col-d">
                                <div class="form-input">
                                    <input id="text" type="text" name="searchQuery" @if($request->has('searchQuery')) value="{{$request->searchQuery}}" @endif placeholder="{{__('adnetwork.page')}}">
                                </div>
                            </div>
                            <div class="col-item col-d">
                                <div class="form-select">
                                    <select name="status" id="country" class="select-ns" data-placeholder="{{__('adnetwork.all')}}">
                                        ...
                                        <option value="active" {{selected_exist($request, 'status_id', 'active')}}>{{__('adnetwork.active')}}</option>
                                        <option value="inactive" {{selected_exist($request, 'status_id', 'inactive')}}>{{__('adnetwork.inactive')}}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-item col-d">
                                <div class="form-select">
                                    <select name="page_id" id="fb_pages" style="width: 100%"  data-live-search="true">
                                        @if(count($page_api)>0)
                                            <option value="{{$page_api['id']}}" selected>{{$page_api['title']}}</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
        ...
        <div class="a-block">
            <div class="a-block-head">{{__('adnetwork.users')}}
                <a href="{{route('fb_user.create', app()->getLocale())}}" class="a-button b-gr f-right with-icon add b-small">{{__('adnetwork.add_user')}}</a>
            </div>
            <div class="a-block-body">
                @if(count($items)>0)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div class="tools-list">
                                            <ul>
                                                <li class="edit"><a class="dropdown-item" href="">{{__('adnetwork.edit')}}</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                    <td>{{$item['name']}}</td>
